import { enviarEventoGA } from './analytics.js';
...
function normalizeIcaoCode(value) {
  return String(value ?? "").trim().toUpperCase();
}

function isValidIcaoCode(value) {
  return ICAO_CODE_PATTERN.test(value);
}

function getProviderForIcao(icao) {
  return icao.startsWith("S") ? "REDEMET" : "NOAA";
}

function sendMetarApiErrorEvent({ provider, icao, code }) {
  enviarEventoGA('erro_api_metar', {
    provedor_utilizado: provider,
    icao_codigo: icao,
    codigo_erro: String(code || "UNKNOWN_ERROR")
  });
}
...
function initAirportSearch() {
  ...
  resultsContainer.addEventListener("click", handleAirportResultClick);
...
function handleAirportResultClick(event) {
  const copyButton = event.target.closest("[data-icao-copy]");

  if (!copyButton || copyButton.disabled) {
    return;
  }

  handleCopyIcao(copyButton);
}
...
async function fetchAirportResults(query, options = {}) {
  ...
  if (!data.success) {
    ...
    error.status = data.status || data.code || response.status;
    throw error;
  }
...
function createAirportCard(airport) {
  const operationTypeLabel = getOperationalTypeLabel(airport);
  const icaoCode = normalizeIcaoCode(airport.display_icao || airport.ident);
  const hasCopyableIcao = isValidIcaoCode(icaoCode);
  const displayCode = hasCopyableIcao
    ? icaoCode
    : getTextValue(airport.display_icao || airport.ident, t("airports.label.icaoUnavailable"));
  const airportName = getTextValue(airport.name);
  const municipality = getTextValue(airport.municipality);
  ...
  card.innerHTML = `
    <div class="flex flex-wrap items-center gap-2 border-b border-slate-200 pb-3 airport-card-header">
      <div class="flex flex-wrap items-center gap-2 airport-code-group">
      ...
      <button
        type="button"
        class="airport-copy-button"
        data-icao-copy="${escapeHtml(icaoCode)}"
        aria-label="${escapeHtml(t("airports.copy.aria"))}"
        title="${escapeHtml(t("airports.copy.aria"))}"
        ${hasCopyableIcao ? "" : "disabled"}
      >
        ${escapeHtml(t("airports.copy.button"))}
      </button>
      <div class="flex flex-wrap items-center gap-2">
        ${iataBadge}
      </div>
      <span class="airport-type-badge rounded-full bg-slate-200 px-2.5 py-0.5 text-xs font-semibold text-slate-700 uppercase tracking-wide ml-0 md:ml-auto self-start md:self-auto">
...
async function handleCopyIcao(copyButton) {
  const icaoCode = normalizeIcaoCode(copyButton.dataset.icaoCopy);

  if (!isValidIcaoCode(icaoCode)) {
    return;
  }

  const defaultLabel = t("airports.copy.button");

  try {
    await copyTextToClipboard(icaoCode);
    copyButton.textContent = t("airports.copy.copied");
  } catch (error) {
    copyButton.textContent = t("metar.copy.select");
  }
...
function renderAirportValidationError(message) {
  resultsContainer.innerHTML = "";
  statusMessage.classList.remove("hidden");
  hideAllStatusStates();
  currentRenderedResults = null;
