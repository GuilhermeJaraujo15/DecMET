  ...
  try {
    ...
    if (response.fallback) {
      sendMetarApiErrorEvent({
        provider: response.fallback.from,
        icao,
        code: response.fallback.status || response.fallback.code
      });
    }
    ...
    sendMetarApiErrorEvent({
      provider: getProviderForIcao(icao),
      icao,
      code: error.status || error.message || "NETWORK_ERROR"
    });
...
function sendMetarApiErrorEvent({ provider, icao, code }) {
  enviarEventoGA('erro_api_metar', {
    provedor_utilizado: provider || getProviderForIcao(icao),
    icao_codigo: icao,
