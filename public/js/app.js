...
const GA4_TEXT_PARAM_LIMIT = 100;
...
function normalizeMetarText(value) {
  return String(value ?? "").trim().replace(/\s+/g, " ").toUpperCase();
}

function limitGA4TextParam(value) {
  return value.length > GA4_TEXT_PARAM_LIMIT
    ? value.slice(0, GA4_TEXT_PARAM_LIMIT)
    : value;
}

function getMetarTokens(metarText) {
  return metarText ? metarText.split(/\s+/) : [];
}

function getIcaoFromMetar(tokens) {
  const firstToken = tokens[0] === "METAR" || tokens[0] === "SPECI"
    ? tokens[1]
    : tokens[0];

  return ICAO_CODE_PATTERN.test(firstToken || "") ? firstToken : "UNKNOWN";
}

function buildMetarAnalyticsParams(metarText) {
  const tokens = getMetarTokens(metarText);

  return {
    metar_completo: limitGA4TextParam(metarText),
    metar_truncado_ga4: metarText.length > GA4_TEXT_PARAM_LIMIT,
    metar_caracteres: metarText.length,
    token_count: tokens.length,
    icao_codigo: getIcaoFromMetar(tokens),
    tipo_reporte: tokens[0] === "SPECI" ? "SPECI" : "METAR",
    possui_cavok: tokens.includes("CAVOK"),
    possui_tempo_recente: tokens.some(token => token.startsWith("RE"))
  };
}
...
function initPageInteractions() {
  ...
  metarForm.addEventListener("submit", function (event) {
    ...
    const metarText = normalizeMetarText(metarInput.value);

    // GA4 receives sanitized values before decoding, so invalid METAR attempts are still measurable.
    enviarEventoGA('decodificacao_metar', buildMetarAnalyticsParams(metarText));
